...
export default function Page(){
  ...
  const load=()=> api.get("/timeline").then(r=>setRows(Array.isArray(r.data)?r.data:r.data?.data||[])).catch(()=>{}).finally(()=>setLoading(false));
  useEffect(()=>{load();},[]);
  const save=async()=>{
    try{
      if(editId) await api.put("/timeline/"+editId, form);
  ...
  const del=async()=>{ await api.delete("/timeline/"+delId); setDelId(null); load(); setToast("تم الحذف"); setTimeout(()=>setToast(""),2000); };
  const baseCols=[{"key": "year", "label": "السنة"}, {"key": "title", "label": "العنوان"}, {"key": "isActive", "label": "الحالة"}];
  const cols=baseCols.map(c=> ({...c, render: c.key==="isActive" ? (v,row)=> <ToggleSwitch checked={!!v} onChange={async(val)=>{ await api.put("/timeline/"+row._id,{isActive:val}); load();}} /> : undefined }));
  if(loading) return <div className="p-8 text-center">جاري التحميل...</div>;
  return <div className="space-y-4">
    <div className="flex justify-between items-center"><h1 className="text-2xl font-bold">الجدول الزمني</h1><button onClick={()=>{setForm({});setEditId(null);setShowForm(true);}} className="btn-primary">+ إضافة</button></div>
    ...
    {showForm && <div className="fixed inset-0 bg-black/40 grid place-items-center z-50 p-4"><div className="bg-white rounded-xl p-6 w-full max-w-lg space-y-3">
      <h3 className="font-bold">{editId?"تعديل":"إضافة"} الجدول الزمني</h3>
      <input value={form.year||""} onChange={e=>setForm({...form,year:e.target.value})} placeholder="السنة" className="border w-full p-3 rounded-lg" />
      <input value={form.title||""} onChange={e=>setForm({...form,title:e.target.value})} placeholder="العنوان" className="border w-full p-3 rounded-lg" />
      <input value={form.description||""} onChange={e=>setForm({...form,description:e.target.value})} placeholder="الوصف" className="border w-full p-3 rounded-lg" />
