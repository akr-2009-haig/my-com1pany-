...
export default function Page(){
  ...
  const load=()=> api.get("/packages").then(r=>setRows(Array.isArray(r.data)?r.data:r.data?.data||[])).catch(()=>{}).finally(()=>setLoading(false));
  useEffect(()=>{load();},[]);
  const save=async()=>{
    try{
      if(editId) await api.put("/packages/"+editId, form);
  ...
  const del=async()=>{ await api.delete("/packages/"+delId); setDelId(null); load(); setToast("تم الحذف"); setTimeout(()=>setToast(""),2000); };
  const baseCols=[{"key": "name", "label": "اسم الباقة"}, {"key": "priceMonthly", "label": "السعر"}, {"key": "isPopular", "label": "مميزة"}];
  const cols=baseCols.map(c=> ({...c, render: c.key==="isActive" ? (v,row)=> <ToggleSwitch checked={!!v} onChange={async(val)=>{ await api.put("/packages/"+row._id,{isActive:val}); load();}} /> : undefined }));
  if(loading) return <div className="p-8 text-center">جاري التحميل...</div>;
  return <div className="space-y-4">
    <div className="flex justify-between items-center"><h1 className="text-2xl font-bold">إدارة الباقات</h1><button onClick={()=>{setForm({});setEditId(null);setShowForm(true);}} className="btn-primary">+ إضافة</button></div>
    ...
    {showForm && <div className="fixed inset-0 bg-black/40 grid place-items-center z-50 p-4"><div className="bg-white rounded-xl p-6 w-full max-w-lg space-y-3">
      <h3 className="font-bold">{editId?"تعديل":"إضافة"} إدارة الباقات</h3>
      <input value={form.name||""} onChange={e=>setForm({...form,name:e.target.value})} placeholder="اسم الباقة" className="border w-full p-3 rounded-lg" />
      <input value={form.priceMonthly||""} onChange={e=>setForm({...form,priceMonthly:e.target.value})} placeholder="السعر الشهري" className="border w-full p-3 rounded-lg" />
      <input value={form.priceYearly||""} onChange={e=>setForm({...form,priceYearly:e.target.value})} placeholder="السعر السنوي" className="border w-full p-3 rounded-lg" />
