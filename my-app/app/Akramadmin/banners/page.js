...
export default function Page(){
  ...
  const load=()=> api.get("/banners").then(r=>setRows(Array.isArray(r.data)?r.data:r.data?.data||[])).catch(()=>{}).finally(()=>setLoading(false));
  useEffect(()=>{load();},[]);
  const save=async()=>{
    try{
  ...
  const del=async()=>{ await api.delete("/banners/"+delId); setDelId(null); load(); setToast("تم الحذف"); setTimeout(()=>setToast(""),2000); };
  const baseCols=[{"key": "page", "label": "الصفحة"}, {"key": "title", "label": "العنوان"}];
  const cols=baseCols.map(c=> ({...c, render: c.key==="isActive" ? (v,row)=> <ToggleSwitch checked={!!v} onChange={async(val)=>{ await api.put("/banners/"+row._id,{isActive:val}); load();}} /> : undefined }));
  if(loading) return <div className="p-8 text-center">جاري التحميل...</div>;
  return <div className="space-y-4">
    <div className="flex justify-between items-center"><h1 className="text-2xl font-bold">بانرات الصفحات</h1><button onClick={()=>{setForm({});setEditId(null);setShowForm(true);}} className="btn-primary">+ إضافة</button></div>
    ...
    {showForm && <div className="fixed inset-0 bg-black/40 grid place-items-center z-50 p-4"><div className="bg-white rounded-xl p-6 w-full max-w-lg space-y-3">
      <h3 className="font-bold">{editId?"تعديل":"إضافة"} بانرات الصفحات</h3>
      <input value={form.page||""} onChange={e=>setForm({...form,page:e.target.value})} placeholder="الصفحة (about, services...)" className="border w-full p-3 rounded-lg" />
