...
export default function Page(){
  ...
  const load=()=> api.get("/menus").then(r=>setRows(Array.isArray(r.data)?r.data:r.data?.data||[])).catch(()=>{}).finally(()=>setLoading(false));
  useEffect(()=>{load();},[]);
  const save=async()=>{
    try{
      if(editId) await api.put("/menus/"+editId, form);
  ...
  const del=async()=>{ await api.delete("/menus/"+delId); setDelId(null); load(); setToast("تم الحذف"); setTimeout(()=>setToast(""),2000); };
  const baseCols=[{"key": "title", "label": "العنوان"}, {"key": "url", "label": "الرابط"}, {"key": "location", "label": "الموقع"}];
  const cols=baseCols.map(c=> ({...c, render: c.key==="isActive" ? (v,row)=> <ToggleSwitch checked={!!v} onChange={async(val)=>{ await api.put("/menus/"+row._id,{isActive:val}); load();}} /> : undefined }));
  if(loading) return <div className="p-8 text-center">جاري التحميل...</div>;
  return <div className="space-y-4">
    <div className="flex justify-between items-center"><h1 className="text-2xl font-bold">إدارة القوائم</h1><button onClick={()=>{setForm({});setEditId(null);setShowForm(true);}} className="btn-primary">+ إضافة</button></div>
    ...
    {showForm && <div className="fixed inset-0 bg-black/40 grid place-items-center z-50 p-4"><div className="bg-white rounded-xl p-6 w-full max-w-lg space-y-3">
      <h3 className="font-bold">{editId?"تعديل":"إضافة"} إدارة القوائم</h3>
      ...
      <input value={form.url||""} onChange={e=>setForm({...form,url:e.target.value})} placeholder="الرابط" className="border w-full p-3 rounded-lg" />
      <input value={form.location||""} onChange={e=>setForm({...form,location:e.target.value})} placeholder="الموقع (header/footer)" className="border w-full p-3 rounded-lg" />
